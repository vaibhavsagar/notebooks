...
newtype SucCont s t r = SucCont
    { unSucCont :: r -> XorCont s t -> AltCont s t -> Parser s t }
newtype XorCont s t = XorCont
    { unXorCont :: AltCont s t -> ParseResult s t }
type AltCont s t = ParseResult s t

cSymbol :: Eq s => s -> CParser s t s
cSymbol sym = CParser $ \sc xc ac -> Parser $ \ss -> case ss of
    (s:ss) | s == sym -> unParser (unSucCont sc sym xc ac) ss
    _                 -> unXorCont xc ac

cSatisfy :: (s -> Bool) -> CParser s t s
cSatisfy pred = CParser $ \sc xc ac -> Parser $ \ss -> case ss of
    (s:ss) | pred s -> unParser (unSucCont sc s xc ac) ss
    _               -> unXorCont xc ac
...
instance Applicative (CParser s t) where
    ...
    pure x = CParser $ \(SucCont sc) -> sc x
...
(<<!>>) (CParser p1) (CParser p2) = CParser $ \sc xc ac -> Parser $ \ss ->
    unParser
        (p1
            (SucCont $ \x xc2 -> unSucCont sc x xc)
            (XorCont $ \ac3 -> unParser (p2 sc xc ac3) ss)
            ac)
        ss

cBegin :: CParser s t t -> Parser s t
cBegin (CParser p) = p
    (SucCont $ \x xc ac -> Parser $ \ss ->
        ParseResult ((ss,x):unParseResult (unXorCont xc ac)))
    (XorCont id)
    (ParseResult [])

cut :: CParser s t r -> CParser s t r
cut (CParser p) = CParser $ \sc xc ac -> p sc (XorCont id) ac

infixr 6 !>>=
(!>>=) :: CParser s t u -> (u -> CParser s t v) -> CParser s t v
(!>>=) (CParser p1) f = CParser $ \sc ->
    p1 (SucCont $ \t ac2 -> unCParser (f t) sc (XorCont id))
...
cListP (CParser p) l = clp l
    where
        clp l = CParser $ \sc xc ac -> Parser $ \ss -> unParser
            (p
                (SucCont $ \r xc2 -> unCParser (clp (r:l)) sc (XorCont id))
                (XorCont $ \ac4 -> unParser (unSucCont sc (reverse l) xc ac4) ss)
                ac)
            ss
